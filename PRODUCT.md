...
- **Gift** takes nothing at the till. As a non-profit the Priory cannot turn the exchange into income, so the stamps are handed over, the donor sends money separately, and the row is logged with its quantities at a Total Amount of 0. That payment is reconciled to the row by the buyer's name afterwards, which is why the name is mandatory for a gift and optional for everything else.
